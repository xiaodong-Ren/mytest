...
def initial_population(p_size,para_num,lower_bound,upper_bound):
    ...
    for i in range(0,p_size):
        ...
        for j in range(0,para_num):
            lb = lower_bound[j]
            ub = upper_bound[j]
            temp.append(random.uniform(lb,ub))#random.uniform(lb,ub)获取lb,ub之间随机数
        ind = individual(temp)
...
def tournment_select(prarents,part_num=2): # binary tournment selection
    ...
    participants = random.sample(prarents, part_num)#随机取2个元素
    best = participants[0]
    best_rank = participants[0].front_rank
    best_crowding_distance = participants[0].crowding_distance
    
    for p in participants[1:] :
        if p.front_rank < best_rank or (p.front_rank == best_rank and p.crowding_distance > best_crowding_distance):
            best = p
...
def cross(p1,p2,prob): # the random linear operator
    ...
    for j in range(0,len(p1.parameter)):
        parameter1.append(alpha*p1.parameter[j] + (1-alpha)*p2.parameter[j] )
        parameter2.append((1-alpha)*p1.parameter[j] + alpha*p2.parameter[j] )
...
def mutation(p,prob): # uniform random mutation
    ...
    for i in range(0,len(p.parameter)):
        if random.uniform(0,1) < prob:
            ...
            if (p.parameter[i]+mutation>parameter_upper_bound[i])or(p.parameter[i]+mutation<parameter_lower_bound[i]):
                if p.parameter[i]+mutation>parameter_upper_bound[i]:
                    parameter.append(parameter_upper_bound[i])
                else:
                    parameter.append(parameter_lower_bound[i])
            else:
...
if __name__ == '__main__':
    ...
    temp = []
    for j in range(0,2):
        lb = parameter_lower_bound[j]
        ub = parameter_upper_bound[j]
